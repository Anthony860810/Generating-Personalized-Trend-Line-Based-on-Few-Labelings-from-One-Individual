...
def DrawTrainTestLoss(train_loss, test_loss):
    ...
    plt.savefig("TrainTestCompareLoss_pretrainfc1.pdf")
    plt.close()
...
if __name__ == '__main__':
    ...
    optimizer = torch.optim.Adamax(model.layer2.parameters(), lr=lr)
    ...
    with torch.no_grad():
        for batch_idx, (data, target) in enumerate(test_loader):
                ...
                for i in range (len(data)):
                    ...
                    Draw(time_series.cpu().detach().numpy(), target[i].cpu().detach().numpy(), y_hat[i].cpu().detach().numpy(), idx, "pretrainfc")
                    idx+=1
