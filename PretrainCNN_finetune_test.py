    ...
    with torch.no_grad():
        for batch_idx, (data, target) in enumerate(test_loader):
                ...
                for i in range (len(data)):
                    ...
                    Draw(time_series.cpu().detach().numpy(), target[i].cpu().detach().numpy(), y_hat[i].cpu().detach().numpy(), idx, "pretraincnn")
                    idx+=1
