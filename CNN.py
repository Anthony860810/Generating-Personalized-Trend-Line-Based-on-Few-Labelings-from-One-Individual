...
class ConvNet(nn.Module):
    def __init__(self):  # override __init__
        ...
        self.layer1 = nn.Sequential(
            ...
            nn.ReLU(),
            nn.Conv1d(20, 40, kernel_size=3),
            nn.ReLU(),
        )
        self.fc = nn.Sequential(
            nn.Linear(67040,1680)
        )
    def forward(self, x):
        out = self.layer1(x)
...
def ParseInput():
    ...
    _parser.add_argument("--epoch", default = 500, help = "Epoch", type=int)
    _parser.add_argument("--lr", default=0.01, help="Learning rate", type=float)
    ...
    with torch.no_grad():
        for batch_idx, (data, target) in enumerate(test_loader):
                ...
                for i in range (len(data)):
                    ...
                    Draw(time_series.cpu().detach().numpy(), target[i].cpu().detach().numpy(), y_hat[i].cpu().detach().numpy(), idx, "cnn")
                    idx+=1
    print("## CNN")
    print("User: ", user)
